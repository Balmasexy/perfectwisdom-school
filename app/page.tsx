  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-20 flex w-64 flex-col border-r border-border bg-sidebar px-4 py-5 transition-transform lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="mt-6 flex flex-1 flex-col gap-1" aria-label="Main navigation">
        {items.map((item) => { const Icon = item.icon; return <button key={item.label} onClick={() => { setActive(item.label); setOpen(false); if (item.label === 'Students') router.push('/students') }} className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${active === item.label ? 'bg-primary text-primary-foreground' : 'text-sidebar-foreground hover:bg-sidebar-accent'}`}><span className="flex items-center gap-3"><Icon aria-hidden="true" />{item.label}</span>{item.count && <span className={`rounded-full px-2 py-0.5 text-xs ${active === item.label ? 'bg-primary-foreground/15' : 'bg-secondary'}`}>{item.count}</span>}</button> })}
      </nav>
